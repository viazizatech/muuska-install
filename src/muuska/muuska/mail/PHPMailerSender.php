...
class PHPMailerSender implements MailSender
{
    ...
    public function __construct($method){
        $this->method = $method;
        $this->defaultConfig = \muuska\config\source\MailConfiguration::getInstance();
    }
    
    protected function loadPHPMailer() {
        if(!self::$phpMailerLoaded){
            App::getApp()->autoloadLibrary('PHPMailer\\PHPMailer', 'vendor\\phpmailer');
    ...
    protected function initMailerMethod(\PHPMailer\PHPMailer\PHPMailer $mailer, \muuska\config\Configuration $mailConfig, Mail $mail) {
        if($this->method === 'smtp'){
            ...
            $mailer->Host = $mailConfig->getString('smtp_host');
            //Set the SMTP port number - likely to be 25, 465 or 587
            $mailer->Port = $mailConfig->getInt('smtp_port', 465);
            //Whether to use SMTP authentication
            $mailer->SMTPAuth = $mailConfig->getBool('smtp_auth');
            //Username to use for SMTP authentication
            $mailer->Username = $mailConfig->getString('smtp_username');
            //Password to use for SMTP authentication
            $mailer->Password = $mailConfig->getString('smtp_password');
